...
static void print_buf(const char* label, const krc_input_unicode_t& ctx)
{
...
static void init_ctx(krc_input_unicode_t& ctx, krc_wchar_t* buf, krc_size_t size,
                     krc_bool_t multiline = KRC_FALSE)
{
    krc_input_unicode_init(&ctx, buf, size, multiline);
}

static void set_hangul(krc_input_unicode_t& ctx)
{
    if (ctx.keyboard_set != KRC_INPUT_KEYBOARD_SET_HANGUL)
        krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_HANGUL);
}

static void shift_on(krc_input_unicode_t& ctx)
{
    if (ctx.shift_state == KRC_FALSE)
        krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_LSHIFT);
}

static void shift_off(krc_input_unicode_t& ctx)
{
...
static void test_latin_basic()
{
    ...
    krc_wchar_t buf[64] = {};
    krc_input_unicode_t ctx;
    init_ctx(ctx, buf, 64);

    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_H);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_E);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_L);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_L);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_O);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_SPACE);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_1);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_2);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_3);
    ...
    // 특수문자
    init_ctx(ctx, buf, 64);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_GRAVE);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_MINUS);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_EQUAL);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_SEMICOLON);
    print_buf("`-=;", ctx);
...
static void test_latin_shift()
{
    ...
    shift_on(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_H);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_I);
    shift_off(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_SPACE);
    shift_on(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_1);
...
static void test_hangul_syllable()
{
    ...
    // R(ㄱ) -> 자음 단독, 조합 시작
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R);
    check("R(ㄱ): composing=true", ctx.hangul_composing == KRC_TRUE);
    check("R(ㄱ): cursor==0", ctx.cursor == 0);

    // K(ㅏ) -> 가 조합
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K);
    print_buf("가", ctx);
    ...
    init_ctx(ctx, buf, 64);
    set_hangul(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_D);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_J);
    print_buf("어", ctx);
...
static void test_hangul_jongseong()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_S);
    print_buf("간", ctx);
...
static void test_hangul_split_jongseong()
{
    ...
    init_ctx(ctx, buf, 64);
    set_hangul(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_G);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_S);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_M);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_F);
    print_buf("한글", ctx);
...
static void test_hangul_compound_jongseong()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_T);
...
static void test_hangul_compound_jungseong()
{
    ...
    set_hangul(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_S);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_N);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_P);
    print_buf("눼(ㅞ)", ctx);
...
static void test_hangul_shift()
{
    ...
    set_hangul(ctx);
    shift_on(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_Q);
    shift_off(ctx);
...
static void test_backspace()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_S);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_BACKSPACE);
    print_buf("간->가", ctx);
...
static void test_cursor_movement()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_A);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_B);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_C);
    check("abc: cursor==3", ctx.cursor == 3);

    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_LEFT);
    check("left: cursor==2", ctx.cursor == 2);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_LEFT);
    check("left: cursor==1", ctx.cursor == 1);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_RIGHT);
    check("right: cursor==2", ctx.cursor == 2);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_HOME);
    check("home: cursor==0", ctx.cursor == 0);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_END);
    ...
    set_hangul(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K); // 가(composing, cursor=0)
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_LEFT);
...
static void test_insert_overwrite()
{
    ...
    // 덮어쓰기 + HOME -> 'x' -> "xbc"
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_INSERT);
    check("INSERT: insert_mode=false", ctx.insert_mode == KRC_FALSE);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_HOME);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_X);
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_Y);
...
static void test_buffer_limit()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_C);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_D);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_E); // 5번째 -> 거부
...
static void test_cursor_pos_singleline()
{
    ...
    set_hangul(ctx);
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_R); // ㄱ
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_K); // 가
...
static void test_cursor_pos_multiline()
{
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_ENTER); // \r\n 삽입, cursor 전진 2
    ...
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_UP);
    ...
    // Down: line 0 → line 1
    krc_input_unicode_put_key(&ctx, KRC_INPUT_KEY_DOWN);
